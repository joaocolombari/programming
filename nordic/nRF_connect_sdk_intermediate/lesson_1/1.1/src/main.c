...
static void timer0_handler(struct k_timer *dummy)
{
    ...
    flip = !flip;
	//k_msleep(2000);
	// Como isso roda como interrupcao (a funcao foi definida como
	// experity function), eu nao posso usar API de thread. Se usa
	// esse k_msleep a aplicacao crasha
}

/* STEP 2.1 - Define the timer */
K_TIMER_DEFINE(timer0, timer0_handler, NULL);
// Parametros sao o nome timer0, a funcao experity function e 
// stop fn -Function to invoke if the timer is stopped while running
...
// As mensagens que vao ser passadas entre as threads

/* STEP 3.2 - Define the message queue */
K_MSGQ_DEFINE(device_message_queue, sizeof(SensorReading), 16, 4);
// Parametros:
/*
1 - Name of the message queue.
2 - Message size (in bytes)
3 - Maximum number of messages that can be queued
4 - Alignment of the message queue's ring buffer
*/
...
int main(void)
{
	...
	// Parametros sao o endereco do timer que eu defini no K_TIMER_DEFINE
	// A duracao e o periodo, respectivamente
